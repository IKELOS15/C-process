...
void array_print_all(int* array, int* name, int size) //전체 출력
{
    printf("---------전체 잔여 수량---------\n");
    for(int i = 0; i<size; i++)
    {
        printf("ID(%d): %s %d개 ", i+1,  *(name+i), *(array+i));
    }
    printf("\n");
    printf("\n");
}
...
void item_naming(char* array, int id_num)  //상품이름 입력
{
    
    int temp_ch;
    int i = 0;
    printf("ID(%d) 이름: ", id_num);
...
int main()
{
    ...
    do{
        ...
        if(choice_num == 1){
            ...
            array_print_all(leftover_item, item_name[0], num);
